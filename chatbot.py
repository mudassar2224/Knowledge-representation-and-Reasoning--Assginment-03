...
# Fixes:
#   1. _all_about() now shows Father, Mother correctly using both arg orders
#   2. _all_about() now shows Children, Ancestors, Descendants
#   3. _answer_relationship() argument order fixed for ancestor/descendant
#   4. Removed unnecessary neo4j reload in _handle_facts_ready to avoid
#      Streamlit rerun freeze (caller does st.rerun() once after)
...
KNOWN_CITIES = {"lahore", "karachi", "islamabad", "peshawar", "hafizabad",
                "soon valley", "multan", "faisalabad", "rawalpindi"}
KNOWN_OCCUPATIONS = {
    "doctor", "teacher", "engineer", "nurse", "lawyer", "accountant",
    "businessman", "professor", "principal", "pilot", "student",
    "farmer", "hacker", "uk",   # uk was entered as occupation in your demo
}
...
def _handle_facts_ready() -> str:
    ...
    # FIX: sync KNOWN_NAMES without full driver reset (avoids Streamlit freeze)
    try:
        from neo4j_engine import get_all_people
        import utils
...
# ═══════════════════════════════════════════════════════════════════════════
# STANDARD FAMILY QUERY DISPATCH
# ═══════════════════════════════════════════════════════════════════════════
...
def _answer_relationship(relation, person):
    # FIX: ancestor(X, person) = who are person's ancestors
    #      ancestor(person, X) = who is person an ancestor of
    # For most relations "X is relation OF person" → query(relation, ["X", person])
    # But for ancestor/descendant the direction matters:
    if relation == "ancestor":
        results = _dedupe(query("ancestor", ["X", person]))
        return format_response("ancestor", person, results)
    if relation == "descendant":
        results = _dedupe(query("descendant", ["X", person]))
        return format_response("descendant", person, results)
    results = _dedupe(query(relation, ["X", person]))
    return format_response(relation, person, results)
